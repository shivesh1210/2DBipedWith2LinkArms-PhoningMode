...
q_bras_fin = Optim_Parameters(8:9);     % final joint angles of the arms without phone(q6 and q7)
q_bras_int = Optim_Parameters(10:11);   % intermediate joint angles of the arms without phone(q6 and q7)
...
% Condition on arm position for Phoning mode
q_bras_fin = [q_bras_fin; pi/2-0.2; 3*pi/2-0.35]; 
q_bras_int = [q_bras_int; pi/2-0.2; 3*pi/2-0.35]; 

% Condition on arm velocity for Phoning mode
